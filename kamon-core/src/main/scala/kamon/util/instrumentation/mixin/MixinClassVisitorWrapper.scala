...
case class MixinClassVisitorWrapper(mixin:MixinDescription) extends ClassVisitorWrapper {
  override def mergeWriter(flags: Int): Int = flags
  override def mergeReader(flags: Int):  Int = flags | ClassReader.EXPAND_FRAMES
  override def wrap(classVisitor: ClassVisitor): ClassVisitor = new MixinClassVisitor(mixin, classVisitor)
}
